...
// Firebase configuration - UPDATE THIS FOR YOUR NETLIFY DEPLOYMENT
...
// Admin credentials
const ADMIN_EMAIL = "admin@askearn.com";
...
if (adminLoginBtn) {
    adminLoginBtn.addEventListener('click', async (e) => {
        ...
        try {
            ...
            // Redirect to admin dashboard (or regular dashboard for now)
            window.location.href = 'dashboard.html';
        } catch (error) {
            const errorCode = error.code;
            const errorMessage = error.message;
            ...
            if (errorCode === 'auth/user-not-found') {
                ...
                showError("Admin login error: " + errorMessage);
            }
        }
    });
}
...
onAuthStateChanged(auth, (user) => {
    ...
    if (window.location.pathname.includes('index.html') || window.location.pathname === '/' || window.location.pathname === '/Askearn/') {
        if (user) {
